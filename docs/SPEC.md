...
- Parent "attempt" button works here too, through the whole game, the play button included, for when detection missed a quiet attempt or the child echoes an item. In an open pause a tap ends it as `pause_parent_credit` with payload `inPause: true`; at any other moment it is logged as `pause_parent_credit` with `inPause: false` against the item the app said last, and the game goes on as it was. Holding it records the sound itself, exactly as on the request board, logged as `attempt_recorded` against the item the open pause waits on, or else the item said last, as they stood when the hold began (`sequence_id` + `item_position`).
...
- Threshold = baseline + margin (default 12 dB, setting). Baseline = median dB of a measurement of the room, taken only in the app's quiet moments, where nothing counts as the child:
  - 2 s while the game waits on its play button, on screen and with a sequence to play;
...
- Starting a session asks for the microphone, so the system dialog comes up in parent mode and never in front of the child. The dialog says all three uses: the parent's voice for cards, noticing the child's sound in the game (nothing of it kept), and the attempts the parent records. Expo Go shows its own text instead; the app's is in the TestFlight build. A refusal only means the session runs without listening: the pause game then ends its pauses on the timer and on the parent's button.
